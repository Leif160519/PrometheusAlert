...
func SendMessageZabbix(message ZabbixMessage, logsign string) string {
	...
	//workwechat
	case "workwechat":
		ret = ret + SendWorkWechat(beego.AppConfig.Int("WorkWechat_AgentID"),beego.AppConfig.String("WorkWechat_AgentSecret"),beego.AppConfig.String("WorkWechat_ToUser"), beego.AppConfig.String("WorkWechat_ToParty"), beego.AppConfig.String("WorkWechat_ToTag"), message.ZabbixMessage, logsign)
	//百度Hi(如流)
	case "rl":
		if message.ZabbixTarget == "" {
